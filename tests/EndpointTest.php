...
final class EndpointTest extends TestCase
{
    ...
    public function testMaxItemsTooLargeReturns400(): void
    {
        ...
        $this->assertSame(400, $res["status"]);
        $this->assertStringContainsString("Maximum number of items", $res["body"]);
    }
    ...
    {
        $berlinStart = new DateTime("tomorrow 08:00", new DateTimeZone("Europe/Berlin"));
        $berlinEnd = new DateTime("tomorrow 16:00", new DateTimeZone("Europe/Berlin"));
        ...
        $ics = $this->buildCalendar([
            ...
            $this->vevent(
                "berlin-1",
                "Berlin Event",
                "DTSTART;TZID=Europe/Berlin:" . $berlinStart->format("Ymd\\THis"),
    ...
    public function testResponseIsCachedOnSecondRequest(): void
    {
        ...
        $params = ["url" => $url, "start" => "today", "end" => "today + 2 days"];
    ...
    public function testRequestedRangeExcludesOutOfRangeEvents(): void
    {
        ...
        $ics = $this->buildCalendar([
            $this->vevent(
                ...
                "DTEND:" . (clone $inRange)->modify("+1 hour")->format("Ymd\\THis") . "Z"
            ),
            $this->vevent(
                "out-of-range",
                "Out Of Range",
                "DTSTART:" . $outOfRange->format("Ymd\\THis") . "Z",
                "DTEND:" . (clone $outOfRange)->modify("+1 hour")->format("Ymd\\THis") . "Z"
            ),
        ]);
    ...
    public function testMaxItemsKeepsEarliestEventsAfterSorting(): void
    {
        ...
        $ics = $this->buildCalendar([
            $this->vevent(
                ...
                "DTEND:" . (clone $day3)->modify("+1 hour")->format("Ymd\\THis") . "Z"
            ),
            $this->vevent(
                "e2",
                "Day 2",
                "DTSTART:" . $day2->format("Ymd\\THis") . "Z",
                "DTEND:" . (clone $day2)->modify("+1 hour")->format("Ymd\\THis") . "Z"
            ),
            $this->vevent(
                "e1",
                "Day 1",
                "DTSTART:" . $day1->format("Ymd\\THis") . "Z",
                "DTEND:" . (clone $day1)->modify("+1 hour")->format("Ymd\\THis") . "Z"
            ),
        ]);
        ...
        try {
            ...
            $ics = $this->buildCalendar([
                $this->vevent(
                    ...
                    "No Debug",
                    "DTSTART:" . $start->format("Ymd\\THis") . "Z",
                    "DTEND:" . (clone $start)->modify("+1 hour")->format("Ymd\\THis") . "Z"
                ),
            ]);
    ...
    private function getFrom(HttpTestServer $server, array $params): array
    {
        ...
        $url = $server->url("api/index.php") . ($query !== "" ? "?" . $query : "");
    ...
    private function vevent(string $uid, string $summary, string $dtstart, string $dtend): string
    {
        return "BEGIN:VEVENT\r\n" .
            "UID:{$uid}\r\n" .
            "DTSTAMP:20200101T000000Z\r\n" .
